"""
解析结果规整与 Markdown 导出（src/parsed_reports_merging.py）

主要功能：
- PageTextPreparation：清洗 Docling 解析 JSON，按页合并文本/表格/列表
- 输出规整后的 JSON 至 debug_data/02_merged_reports/{sha1}.json
- 支持导出 Markdown 至 debug_data/03_reports_markdown/
...
如何调用：
  ...
  prep.process_reports(
      reports_dir=Path("debug_data/01_parsed_reports"),
      output_dir=Path("debug_data/02_merged_reports"),
  )
  # 通常由 Pipeline.merge_reports() / export_reports_to_markdown() 调用
"""
import re
...
class PageTextPreparation:
    ...
    def process_report(self, report_data):
        ...
        if total_corrections > 0:
            sha1_label = self.report_data["metainfo"].get("sha1", "unknown")
            print(f"Fixed {total_corrections} occurrences in the file {sha1_label}")
    ...
    def _resolve_processed_report(self, report_data: dict) -> dict:
        """兼容原始解析 JSON 与 merge 后的 JSON。"""
        content = report_data.get("content")
        if isinstance(content, dict) and "pages" in content:
            return content
        return self.process_report(report_data)
    ...
    def export_to_markdown(self, reports_dir: Path, output_dir: Path):
        ...
        for report_path in reports_dir.glob("*.json"):
            ...
            processed_report = self._resolve_processed_report(report_data)

"""
PDF 解析（Docling）（src/pdf_parsing.py）

主要功能：
- 使用 Docling 将 pdf_reports/ 下 PDF 转为结构化 JSON（按页、表格、图片）
- 支持并行解析；Windows 中文路径下自动复制到 ASCII 临时目录再解析
...
如何调用：
  ...
  parser = PDFParser(
      output_dir=Path("debug_data/01_parsed_reports"),
      csv_metadata_path=Path("subset.csv"),
  )
  parser.parse_and_export(doc_dir=Path("pdf_reports"))
  # 通常由 Pipeline.parse_pdf_reports() / main.py parse-pdfs 调用
"""
import os
import time
import logging
...
def _needs_ascii_path_copy(pdf_path: Path) -> bool:
    """docling-parse 在 Windows 非 ASCII 路径下可能加载失败。"""
    return not str(pdf_path.resolve()).isascii()


def _prepare_pdf_paths_for_docling(pdf_paths: List[Path]) -> tuple[List[Path], list[str]]:
    """将 PDF 复制到 ASCII 临时路径后再交给 Docling 解析。"""
    import hashlib
    import shutil
    import tempfile
    ...
    for pdf_path in pdf_paths:
        if not _needs_ascii_path_copy(pdf_path):
            resolved.append(pdf_path)
            continue
        temp_dir = tempfile.mkdtemp()
        temp_dirs.append(temp_dir)
        dest = Path(temp_dir) / f"{hashlib.sha1(pdf_path.read_bytes()).hexdigest()}.pdf"
...
class PDFParser:
    ...
    def _parse_csv_metadata(csv_path: Path) -> dict:
        """解析 subset.csv，以 sha1 为键建立元数据查找表。"""
        import csv
        metadata_lookup = {}
    ...
    def process_documents(self, conv_results: Iterable[ConversionResult]):
        ...
        for conv_res in conv_results:
            if conv_res.status == ConversionStatus.SUCCESS:
                ...
                sha1 = processed_report["metainfo"].get("sha1", conv_res.input.file.stem)
                if self.output_dir is not None:
                    with (self.output_dir / f"{sha1}.json").open("w", encoding="utf-8") as fp:
    ...
    def parse_and_export(self, input_doc_paths: List[Path] = None, doc_dir: Path = None):
        ...
        resolved_paths, temp_dirs = _prepare_pdf_paths_for_docling(input_doc_paths)
        try:
            conv_results = self.convert_documents(resolved_paths)
